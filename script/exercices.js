...
function exo3(){
    ...
    submit.addEventListener("click",function(){
        ...
        if(code.value == correctAnswer){
            ...
            notif.innerHTML = '<div role="alert" class="alert alert-success"><svg xmlns="http://www.w3.org/2000/svg" class="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg><div><h3 class="font-bold">Bravo !</h3><div class="text-xs">Vous avez trouvé la bonne réponse.</div></div><button onclick="end()" class="btn btn-sm">Terminer</button></div>'
            console.log("true");
        }else{
...
function end(){
    location.assign("endscreen/index.html");
}
